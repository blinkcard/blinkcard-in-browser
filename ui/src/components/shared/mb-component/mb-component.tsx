...
import { CheckConclusion, SdkService } from '../../../utils/sdk.service';
...
export class MbComponent {
  ...
  @State() apiProcessStatusState: 'ERROR' | 'LOADING' | 'NONE' | 'SUCCESS' =
    'NONE';
   ...
   * See description in public component.
   */
  @Prop() iconDragAndDropWarningDefault: string =
  ...
  componentDidLoad() {
    ...
    const exportedParts = GenericHelpers.getWebComponentExportedParts(
      this.hostEl.shadowRoot
    );

    this.hostEl.setAttribute(
      'exportparts',
      parts.concat(exportedParts).join(', ')
    );
  ...
  private async init() {
    ...
    const hasMandatoryCapabilities =
      await DeviceHelpers.checkMandatoryCapabilites();

    if (!hasMandatoryCapabilities) {
      this.setFatalError(
        new SDKError(ErrorTypes.componentErrors.browserNotSupported)
      );
      return;
    }
    ...
    const initEvent: EventReady | SDKError = await this.sdkService.initialize(
      this.licenseKey,
      {
    ...
    if (this.scanFromImage) {
      ...
      const imageScanIsAvailable = this.sdkService.isScanFromImageAvailable(
        this.recognizers,
      ...
      if (imageScanIsAvailable) {
        this.imageRecognitionType = this.sdkService.getScanFromImageType(
          this.recognizers,
          this.recognizerOptions
        );
  ...
  private async checkInputProperties(): Promise<boolean> {
    if (!this.licenseKey) {
      this.setFatalError(
        new SDKError(BlinkCardSDK.sdkErrors.licenseKeyMissing)
      );
      return false;
    }

    // Recognizers
    const conclusion: CheckConclusion = this.sdkService.checkRecognizers(
      this.recognizers
    );
    ...
    this.cameraExperience.type = this.sdkService.getDesiredCameraExperience(
      this.recognizers,
      this.recognizerOptions
    );
    return true;
    ...
    const eventHandler = (recognitionEvent: RecognitionEvent) => {
      switch (recognitionEvent.status) {
        ...
        case RecognitionStatus.Ready:
          this.cameraExperience.setActiveCamera(
            this.sdkService.videoRecognizer.deviceId
          );
          break;
  ...
  private async startScanFromImageMultiSide(
    firstFile?: File,
    secondFile?: File
  ) {
  ...
  private handleScanError(error: any) {
    ...
    if (error?.code === BlinkCardSDK.ErrorCodes.LICENSE_UNLOCK_ERROR) {
      this.setFatalError(
        new SDKError(ErrorTypes.componentErrors.licenseError, error)
      );
  ...
  private setDragAndDrop() {
    const dropTarget =
      this.galleryDropType === 'FULLSCREEN' ? window : this.hostEl;
  ...
  private setFatalError(error: SDKError) {
    ...
    if (error.details) {
      switch (error.details?.code) {
        case BlinkCardSDK.ErrorCodes.LICENSE_UNLOCK_ERROR:
          const licenseErrorType = error.details?.type;

          switch (licenseErrorType) {
            case BlinkCardSDK.LicenseErrorType.NetworkError:
  ...
  private openMultiSideGalleryUpload(): void {
    const dialog = this.screens.action.querySelector(
  ...
  private async onMultiSideImageChange(
    ev: FileList,
    imageType: MultiSideImageType
  ) {
    ...
    return (
      <Host>
        ...
        >
          <div class='actions'>
            ...
            <div class='action-buttons'>
              <mb-button
                ref={(el) =>
                  (this.scanFromCameraButton = el as HTMLMbButtonElement)
                }
                visible={true}
                disabled={false}
                ...
                buttonTitle={
                  this.translationService.i('action-alt-camera') as string
                }
              ></mb-button>
              <input
                tabindex='-1'
                ...
                accept='image/*'
                onChange={() =>
                  this.scanFromImageInput.value && this.startScanFromImage()
                }
              />
              <mb-button
                ref={(el) =>
                  (this.scanFromImageButton = el as HTMLMbButtonElement)
                }
                disabled={false}
                visible={false}
                selected={false}
                ...
                buttonTitle={
                  this.translationService.i('action-alt-gallery') as string
                }
              ></mb-button>
            </div>
          </div>
          ...
          <div class='multi-side-image-upload'>
            <div class='image-upload-row'>
              <mb-image-box
                ...
                box-title={this.translationService
                  .i('process-image-box-first')
                  .toString()}
                anchor-text={this.translationService
                  .i('process-image-box-add')
                  .toString()}
                onImageChange={(ev: CustomEvent<FileList>) =>
                  this.onMultiSideImageChange(
                    ev.detail,
                    MultiSideImageType.First
                  )
                }
              ></mb-image-box>
              <mb-image-box
                ref={(el) =>
                  (this.imageBoxSecond = el as HTMLMbImageBoxElement)
                }
                box-title={this.translationService
            ...
            <mb-button-classic
              ref={(el) =>
                (this.multiSideScanFromImageButton =
                  el as HTMLMbButtonClassicElement)
              }
              disabled={true}
              clickHandler={() => this.startScanFromImageMultiSide()}
            >
              {this.translationService.i('process-image-upload-cta').toString()}
            </mb-button-classic>
          </div>
        </mb-screen>
        ...
        >
          ...
          <p class='done'>
            <mb-completed
              icon={this.iconGalleryScanningCompleted}
            ></mb-completed>
            <span>
              {this.translationService
        ...
        >
          <img
            ...
            src={this.iconDragAndDropGalleryDefault}
          />
          <p class='drag-and-drop-message'>
            Whoops, we don't support that image format. Please upload a JPEG or
            PNG file.
          </p>
          <div
            id='drag-and-drop-zone'
            ref={(el) => (this.dragAndDropZone = el as HTMLDivElement)}
          ></div>
        </mb-overlay>
        ...
        >
          <mb-spinner
            icon={this.iconSpinnerFromGalleryExperience}
            size='large'
          ></mb-spinner>
          ...
          >
            <div slot='actionButtons'>
              ...
              >
                {this.translationService.i('modal-window-close').toString()}
              </button>
            </div>
          </mb-modal>
        </mb-overlay>
        ...
        >
          <div class='holder'>
            ...
            <mb-camera-experience
              ref={(el) =>
                (this.cameraExperience = el as HTMLMbCameraExperienceElement)
              }
              ...
              onSetIsCameraActive={(ev: CustomEvent<boolean>) =>
                this.handleSetIsCameraActive(ev.detail)
              }
              onChangeCameraDevice={(ev: CustomEvent<CameraEntry>) =>
                this.changeCameraDevice(ev.detail)
              }
              ...
              helpScreensTooltipPauseTimeout={
                this.helpScreensTooltipPauseTimeout
              }
        ...
        >
          <mb-modal
            ref={(el) =>
              (this.licenseExperienceModal = el as HTMLMbModalElement)
            }
            modalTitle='Error'
          >
            <div slot='actionButtons'>
